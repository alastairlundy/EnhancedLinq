...
public static class ImmediateMemorySelectManyExtensions
{
    ...
    extension<TSource>(Span<TSource> source)
    {
            ...
            foreach (TResult obj in collection)
            {
                if (results.Count == int.MaxValue || collection.Count + results.Count >= int.MaxValue)
                    throw new OverflowException(
                        $"{nameof(results)} contains the maximum size of {int.MaxValue} and cannot be added to.");
        ...
        /// <summary>
        ///     Projects each element of a Span into a collection, applies a projection function to each element of the resulting
        ///     collections, and flattens them into a single array.
        /// </summary>
        /// <param name="collectionSelector">
        ///     A function to extract a collection of intermediate elements from the input Span
        ///     elements.
        /// </param>
        /// <param name="resultSelector">
        ///     A function to transform an element from the source Span and an element of the intermediate
        ///     collection into a final result.
        /// </param>
        ...
        ///     Thrown when the total number of resulting elements exceeds the maximum allowed
        ///     size.
        /// </exception>
        public TResult[] SelectMany<TCollection, TResult>(Func<TSource, ICollection<TCollection>> collectionSelector,
            Func<TSource, TCollection, TResult> resultSelector)
        {
            ArgumentNullException.ThrowIfNull(collectionSelector);
            ArgumentNullException.ThrowIfNull(resultSelector);

            List<TResult> results = new(source.Length);
